...
namespace EpointAutomationHelper
{
    ...
    public static class PopupWatcherLibrary
    {
        ...
        /// 监控控件，出现后执行给定操作
        /// </summary>
        /// <param name="findElement">被监控控件</param>
        /// <param name="callback">控件出现后执行的操作</param>
        /// <returns></returns>
        public static PopupWatcher StartPopupWatcher(RepoItemInfo findElement, PopupItemCallback callback)
        {
            ...
            var watcher = new PopupWatcher();
            watcher.Watch(findElement, callback);
            watcher.Start();
            watchers.Add(key, watcher);
            Report.Info("Popup watcher started.");
            return watcher;
        ...
        /// 停止监视器
        /// </summary>
        /// <param name="findElement">需停止监视的控件</param>
        public static void StopPopupWatcher(RepoItemInfo findElement)
        {
            var key = findElement.GetMetaInfos()["id"];
            PopupWatcher watcher = null;
            if (watchers.TryGetValue(key, out watcher))
